...
ADAMW_LR = float(os.environ.get("ADAMW_LR", "6e-4"))
...
CKPT_DIR = os.environ.get("CKPT_DIR", os.path.join(CACHE, f"checkpoints_tmm_owt{LR_SUFFIX}"))

SEED = int(os.environ.get("SEED", "42"))
VEL_INIT = os.environ.get("VEL_INIT", "embed")
...
def main():
    ...
    if rank == 0:
        print("\n=== Building TMMFormer ===")
    model = TMMFormer(vel_init=VEL_INIT).to(device)
    if rank == 0:
    ...
    if rank == 0:
        config = {
            ...
            "dataset": "OpenWebText",
            "n_layers": 12, "n_heads": 12, "d_model": 768, "vel_init": VEL_INIT,
        ...
        wandb.init(project="tmmformer-owt",
                   name=RUN_NAME or (f"tmm{LR_SUFFIX}" if LR_SUFFIX else None),
