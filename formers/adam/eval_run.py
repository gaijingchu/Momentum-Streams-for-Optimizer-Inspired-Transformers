"""Downstream evaluation of AdamFormer using lm-evaluation-harness v0.4.3."""
...
from formers.adam.eval_model import AdamFormerLM


def main():
    parser = argparse.ArgumentParser(description="Evaluate AdamFormer on downstream tasks")
    parser.add_argument("--checkpoint", default="checkpoints_adam/best.pt", help="Path to checkpoint")
    parser.add_argument("--output-dir", default="eval_results_adam", help="Output directory")
    ...
    lm = AdamFormerLM(checkpoint_path=args.checkpoint, device=args.device)
