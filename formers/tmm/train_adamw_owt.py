"""DDP training script for TMMFormer trained with **pure AdamW** on OpenWebText.

Same architecture as tmm_train_owt.py, but Muon (for 2D weights) is replaced with
AdamW, and the scalar / 1D groups all share the same single AdamW config as
vanilla_adamw_train_owt.py (lr=6e-4, betas=(0.9,0.95), wd_2D=0.1, wd_LN=0).
Pairs with vanilla-AdamW-OWT to isolate "model architecture" from "optimizer rule".
"""
...
# Hyperparameters — match vanilla_adamw_train_owt.py exactly.
TOTAL_STEPS = 30_000
WARMUP_STEPS = 3_000
BATCH_SIZE = 8
TOTAL_GRAD_ACCUM = 60
...
CKPT_DIR = os.path.join(CACHE, f"checkpoints_tmm_adamw_owt{LR_SUFFIX}")
...
def configure_optimizer(model):
    ...
    for name, param in model.named_parameters():
        ...
        if "ln" in name:
            ...
            # Catches biases AND TMM's scalar group ("_raw" gamma/nu).
            ln_params.append(param)
    ...
    print(f"  LN / 1D / scalars: {len(ln_params):>3} params, lr={LR}, wd={WD_LN}")
...
def main():
    ...
    if rank == 0:
        config = {
            ...
            "variant": "tmm-adamw (pure AdamW, no Muon)",
            "dataset": "OpenWebText",
        ...
        wandb.init(project="optformer-factorial-ablation-owt",
                   name=f"tmm-adamw-owt{LR_SUFFIX}", config=config, resume="allow")
