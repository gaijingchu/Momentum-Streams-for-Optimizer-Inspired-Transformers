"""DDP training script for RMSPropFormer (per-coord precond, no momentum) on TinyStories.

Factorial-ablation cell: No-momentum x Per-coordinate preconditioning. Mirrors
adam_train_ddp.py hyperparameters for fair comparison.
"""
...
from formers.rmsprop.model import RMSPropFormer
...
CKPT_DIR = os.path.join(CACHE, "checkpoints_rmsprop_ts")
SEED = 42
...
def main():
    ...
    if rank == 0:
        print("\n=== Building RMSPropFormer ===")
    model = RMSPropFormer().to(device)
    if rank == 0:
    ...
    if rank == 0:
        config = {
            "model": "RMSPropFormer-small",
            "variant": "RMSProp+Lie-Trotter",
            "ablation_cell": "no_momentum x per_coord",
    ...
    if rank == 0:
        ...
        print(f"\n=== Training complete (RMSPropFormer) ===")
