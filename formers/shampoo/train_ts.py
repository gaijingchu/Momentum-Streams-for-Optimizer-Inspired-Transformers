"""DDP training script for ShampooFormer (per-token head-wise R, no momentum) on TS.

Factorial-ablation cell: No-momentum x Full-matrix preconditioning. SOAPFormer
with the first-moment buffer removed; mirrors soap_train_ddp.py hyperparameters
(BATCH_SIZE=4, TOTAL_GRAD_ACCUM=120 for the per-token (D,D) R stream's memory
footprint).
"""
...
from formers.shampoo.model import ShampooFormer

# ── Hyperparameters ──────────────────────────────────────────────────────────
TOTAL_STEPS = 10_000
WARMUP_STEPS = 1_000
BATCH_SIZE = 4              # halved (matches SOAPFormer) due to per-token R
...
CKPT_DIR = os.path.join(CACHE, "checkpoints_shampoo_ts")
SEED = 42
...
def main():
    ...
    if rank == 0:
        print("\n=== Building ShampooFormer ===")
    model = ShampooFormer().to(device)
    if rank == 0:
    ...
    if rank == 0:
        config = {
            "model": "ShampooFormer-small",
            "variant": "Shampoo+Lie-Trotter (SOAP w/o momentum)",
            "ablation_cell": "no_momentum x full_matrix",
        ...
        wandb.init(project="optformer-factorial-ablation",
                   name="shampooformer-ts",
    ...
    if rank == 0:
        ...
        print(f"\n=== Training complete (ShampooFormer) ===")
