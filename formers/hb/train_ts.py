"""DDP training script for HBFormer (Heavy-ball + Lie-Trotter) on TinyStories.

Factorial-ablation cell: Heavy-ball momentum x No preconditioning. Mirrors the
hyperparameters of adam_train_ddp.py / yurii_train_ddp.py for fair comparison.
"""
...
from formers.hb.model import HBFormer
...
CKPT_DIR = os.path.join(CACHE, "checkpoints_hb_ts")
SEED = 42
...
def main():
    ...
    if rank == 0:
        config = {
            "model": "HBFormer-small",
            "variant": "HeavyBall+Lie-Trotter",
            "ablation_cell": "heavy_ball x no_precond",
