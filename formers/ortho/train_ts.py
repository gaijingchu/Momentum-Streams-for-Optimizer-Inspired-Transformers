"""DDP training script for OrthoFormer (per-token head-wise NS, no momentum) on TS.

Factorial-ablation cell: No-momentum x Spectral preconditioning. MuonFormer
with the EMA momentum buffer removed; mirrors muon_train_ddp.py hyperparameters.
"""
...
from formers.ortho.model import OrthoFormer
...
CKPT_DIR = os.path.join(CACHE, "checkpoints_ortho_ts")
SEED = 42
...
def main():
    ...
    if rank == 0:
        print("\n=== Building OrthoFormer ===")
    model = OrthoFormer().to(device)
    if rank == 0:
    ...
    if rank == 0:
        config = {
            "model": "OrthoFormer-small",
            "variant": "Ortho+Lie-Trotter (Muon w/o momentum)",
            "ablation_cell": "no_momentum x spectral",
