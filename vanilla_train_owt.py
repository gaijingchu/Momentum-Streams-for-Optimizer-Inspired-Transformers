...
CKPT_DIR = os.environ.get("CKPT_DIR", os.path.join(CACHE, f"checkpoints_vanilla_owt{LR_SUFFIX}"))
...
def main():
    ...
    model = VanillaTransformer(d_model=D_MODEL, n_layers=N_LAYERS, n_heads=N_HEADS).to(device)
    if rank == 0:
    ...
    if rank == 0:
        ...
        wandb.init(project="vanilla-transformer-owt",
                   name=RUN_NAME or (f"vanilla{LR_SUFFIX}" if LR_SUFFIX else None),
